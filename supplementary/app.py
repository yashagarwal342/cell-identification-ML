...
def create_heatmap(image_path, coords, labels, output_path=None, show=True, quadrant=None):
    ...
    # Prevent matplotlib from auto-compressing the legend area    
    leg.set_in_layout(True)

    # Use constrained_layout to intelligently preserve space
    fig.set_constrained_layout(True)

    ax.axis('off')
    ax.set_title('Prediction Heatmap', fontsize=30)
    # plt.tight_layout()
...
def create_comparison_heatmap(image_path, coords, true_labels, pred_labels, 
    ...
    if quadrant:
        plt.suptitle(f'Comparison of True vs Predicted Labels for {model_name} on {quadrant}', fontsize=30)
    else:
    ...
    ui.navset_card_tab(
        ...
        ui.nav_panel("🧠 Aggregator Evaluation",
            ui.tags.div(
                ...
                ui.tags.strong("⚠️ The first run or running after clearing the cache will be very slow. Please wait patiently.")
            ),
            ui.accordion(
                ui.accordion_panel(
                ...
                ui.accordion_panel(
                    ...
                    ui.layout_sidebar(
                        ui.sidebar(
                            ...
                            # ui.hr(),
                            ui.input_action_button("clear_cache_btn", "🧹 Clear Cache\n(Can't REDO! Reload may take over 10 minutes)", class_="btn-danger")
                        ),
                        ui.layout_columns(
                            ...
                            ui.card(ui.h2("🗺️ Heatmap from CV", class_="text-info text-center"),
                                    ui.p("Image truncated due to incomplete annotation in the original data."),
                            ...
                            ui.card(ui.h2("📊 Distributions"), ui.output_plot("cv_distribution_plot"), full_screen=True, class_="with-accent-purple"),
        ...
        ui.nav_panel("🖼️ Whole Slide Prediction",
            ui.tags.div(
                {"style": "margin-bottom: 16px;"},
                ui.tags.strong("A default sample image is provided, but please upload your own image for prediction."),
                ui.tags.br(),
                ui.tags.strong("⚠️ The first run or running after clearing the cache will be very slow. Please wait patiently.")
            ),
            ui.layout_sidebar(
...
# Display the cross-validation heat map (decision boundary)
    ...
    def cv_metrics_plot():
        ...
        plt.title(f"{input.model_select()} Confusion Matrix on {input.quadrant()}")
    ...
    def get_upload_agg_map(filepath, model_name):
        ...
        # If both aggregation and heatmap exist, return directly
        ...
        # Otherwise, prediction is needed
        ...
        # Generate heatmap while predicting
    ...
    def upload_heatmap():
        if input.image_upload():
            #filepath = input.image_upload()[0]['datapath']
            saved_name = input.image_upload()[0]['name']
    ...
    def upload_model():
        ...
        print(f"✅ Loaded model for {name}.")
        return model, lambda x: x / 255.0, (299, 299) if name == "InceptionV3" else (224, 224)
